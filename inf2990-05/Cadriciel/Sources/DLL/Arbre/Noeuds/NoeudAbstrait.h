...
public:
	...
	/// Affiche le noeud.
	virtual void afficher(const glm::mat4& vueProjection, const bool& estColore) const;
	...
	inline glm::vec3 getCentreDeRotation();
	inline void setCentreDeRotation(glm::vec3 centreDeRotation);
	inline double getAngleRotation();
	inline void setAngleRotation(double angleRotation);
	inline double getFacteurEchelle();
	inline void setFacteurEchelle(double facteurEchelle);
	virtual void setCouleurSel();
	
	///Decalration des fonctions pour la friction, acceleration et le rebond.
	float getCoefficientFriction();
	void setCoefficientFriction(float friction);
	...
	void setCoefficientRebond(double rebond);
	double getCoefficientRebond();

	void setTempsInitial(double nouveauTempsInitial);
	double getTempsInitial();

	void setPositionInitialX(double nouveauX);
	double getPositionInitialX();

	void setPositionInitialY(double nouveauY);
	double getPositionInitialY();

	GLubyte coulSel_[3];
	static int cptSelection_;
	int static compteurB_;
	int static compteurP_;
	int static compteurMa_;
...
protected:
	...
	/// rayon du modele
	double rayon_;
	/// indique si le noeud a subit une modification de sa position ou de sa forme
	bool estModifie_;
	int compteurSelection_;
	
	///Varaible pour le rebond, acceleration et friction.
	float coefficientFriction_;
	double vitesseInitial_;
	double acceleration_;
	double coefficientRebond_;

	double tempsInitial_ = 0;
	double positionInitialX_ = 0;
	double positionInitialY_ = 0;
	...
	glm::vec3 centreDeRotation_;
	double angleRotation_;
	///Attribut pour la selection unique d'objet
	std::vector<GLubyte> couleurDeSelection_;
...
////////////////////////////////////////////////////////////////////////
///
/// @fn inline float NoeudAbstrait::obtenirRayon() const 
///
/// Cette fonction qui retourne l'angle de rotation du noeud
///
/// @return L'angle de rotation du noeud.
///
////////////////////////////////////////////////////////////////////////
//inline GLfloat NoeudAbstrait::obtenirAngleRotation() const
//{
...
///// @fn inline float NoeudAbstrait::obtenirRayon() const 
/////
///// Cette fonction retourne le rayon de ce noeud.
/////
...
inline glm::vec3 NoeudAbstrait::getCentreDeRotation()
{
	return centreDeRotation_;
}

inline void NoeudAbstrait::setCentreDeRotation(glm::vec3 centreDeRotation)
{
	centreDeRotation_ = centreDeRotation;
}

inline double NoeudAbstrait::getAngleRotation()
{
	return angleRotation_;
}

inline void NoeudAbstrait::setAngleRotation(double angleRotation)
{
	angleRotation_ = angleRotation;
}

inline double NoeudAbstrait::getFacteurEchelle()
{
	return facteurEchelle_;
}

inline void NoeudAbstrait::setFacteurEchelle(double facteurEchelle)
{
