...
public class HashFunctions 
{
   ...
   public static void main(String[] args) 
   {
      ...
      // Verifie les proprietes d'occupation d'espace
      System.out.println( "Number of elements: " + al.size() );
      System.out.println("SizeOfData" +pfhash.data.length);
   ...
   {
	   //Il s'agit du plus grand nombre premier qu'une variable de type integer 
	   //peut prendre en raison de la memoire maximal allouee a ce type (2^31 - 1).
	   int p = 46337;
	   //On creer le generateur
	   Random generator = new Random(System.nanoTime());
	   int temp; // Est-ce necessaire?
	   
	   ArrayList<Integer> donneeAleatoire = new ArrayList<Integer>();
	   
	   for(int i=0; i <length; i++){
		   
		   //On donne une valeur aleatoire a temp.
		   temp = generator.nextInt(p);
		   
		   //Si cette valeur est deja dans le tableau, 
		   //on lui assigne une autre jusqu'a ce que 
		   //se ne soit plus le cas.
		   while(donneeAleatoire.contains(temp)){
			   temp = generator.nextInt(p);
		   }
		   donneeAleatoire.add(temp);
	   }
      return donneeAleatoire;
   }
}
